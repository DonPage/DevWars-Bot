...
var normalizeChannel = function (ch) {
    return !ch ? false : ch[0] != '#' ? '#' + ch : ch
};
var first = function (a, f) {
    for (var i = 0, l = a.length; i < l; i++) if (f(a[i])) return a[i];
    return false
};
var merge = function () {
...
var escapeForRegExp = function (c) {
    return '\\' + c
};
...
function TwitchBot(o) {
    ...
    var t = function () {
        var u = this.users();
        return u[0 | u.length * Math.random()]
    };
    this.vars = {
        name: this.name,
        ...
        freecommands: function () {
            return this.getFreeCommands().join(', ')
        },
        user: t,
        randuser: t
    };
...
TwitchBot.prototype.users = function () {
    return Object.keys(this._users)
};
TwitchBot.prototype.mods = function () {
    var r = [];
    for (var name in this._users) if (this.isMod(name)) r.push(name);
    return r
};
TwitchBot.prototype.isMod = function (name) {
    var name = name.toLowerCase();
    return this._users[name] == MOD || this._alwaysMod.indexOf(name) > -1
};
TwitchBot.prototype.addMod = function (name) {
    var name = name.toLowerCase();
    if (this._users[name]) this._users[name] = MOD;
    return this
};
TwitchBot.prototype.removeMod = function (name) {
    var name = name.toLowerCase();
    if (this._users[name]) this._users[name] = NOT_MOD;
    return this
};
TwitchBot.prototype.alwaysMod = function (name) {
    var name = name.toLowerCase();
    this._alwaysMod.push(name);
    return this
};

TwitchBot.prototype.getCommands = function () {
...
TwitchBot.prototype.disableCommand = function (name) {
    this.comdis[name] = true;
    return this
};
TwitchBot.prototype.enableCommand = function (name) {
    this.comdis[name] = false;
    return this
};
...
TwitchBot.prototype.run = function () {
    ...
    this.bot.addListener('names' + this.channel, (function (nicks) {
        this._users = {};
        for (var k in nicks) this._users[k] = true
    }).bind(this));
    this.bot.addListener('join' + this.channel, (function (nick) {
        this.tryJoin(nick);
        this._users[nick] = NOT_MOD
    }).bind(this));
    this.bot.addListener('part' + this.channel, (function (nick) {
        this.tryLeave(nick);
        delete this._users[nick]
    }).bind(this));
    this.bot.addListener('+mode', (function (channel, by, mode, argument, msg) {
        if (mode == 'o') this._users[msg.args[2]] = MOD
    }).bind(this));
    this.bot.addListener('-mode', (function (channel, by, mode, argument, msg) {
        if (mode == 'o') this._users[msg.args[2]] = NOT_MOD
    }).bind(this));
    process.on('SIGINT', this.doExit.bind(this));
    process.on('SIGTERM', this.doExit.bind(this));
    process.on('SIGQUIT', this.doExit.bind(this));
    if (this.debug) this.bot.addListener('error', function (msg) {
        console.log('Error: ', msg)
    });
    return this;
};

TwitchBot.prototype.doMessage = function (text, from, to, msg) {
    return this.tryCommands(from || this.name, to || this.channel, text || '', msg || {})
};
...
TwitchBot.prototype.getVar = function (name) {
    this.vars[name]
};

TwitchBot.prototype.onJoin = function (f) {
    this._join.push(f);
    return this
};
TwitchBot.prototype.onLeave = function (f) {
    this._leave.push(f);
    return this
};

TwitchBot.prototype.onExit = function (f) {
    this._onexit.push(f);
    return this
};
TwitchBot.prototype.addLogger = function (f) {
    this.loggers.push(f);
    return this
};

TwitchBot.prototype.addIntervalMessage = function (msg, time, o) {
    setInterval((function () {
        this.sayWithVars(msg, o)
    }).bind(this), time);
    return this
};
TwitchBot.prototype.addTimeoutMessage = function (msg, time, o) {
    setTimeout((function () {
        this.sayWithVars(msg, o)
    }).bind(this), time);
    return this
...
TwitchBot.prototype.loadPlugins = function (files, a) {
    if (Array.isArray(files)) files.forEach(this.loadPlugin.bind(this));
        ...
        var fl = fs.readdirSync(files).filter(function (x) {
            return /.*\.js/i.test(x)
        }).forEach((function (x) {
...
TwitchBot.prototype.autosave = function () {
    if (this._autosave) this.save(this.file);
    return this
};
